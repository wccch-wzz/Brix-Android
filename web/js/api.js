...
// 后端服务器基地址
// Tauri 混合架构：sidecar HTTP 服务器地址（sidecar 就绪后由 tauri-bridge.js 设置）
// Electron 架构：空字符串（同源请求，通过 brix:// 协议拦截）
let API_BASE = '';

/**
 * 动态获取 API 基地址
 * Tauri 环境下从 electronAPI._getSidecarBase() 获取 sidecar HTTP 地址
 * Electron 环境下返回空字符串（使用 brix:// 协议）
 */
function getApiBase() {
    if (typeof window !== 'undefined' && window.electronAPI && window.electronAPI._getSidecarBase) {
        const base = window.electronAPI._getSidecarBase();
        if (base) return base;
    }
    return API_BASE;
}

// sidecar 就绪后更新 API_BASE（由 tauri-bridge.js 调用）
window._updateApiBase = function (base) { API_BASE = base; };
...
async function fetchWithTimeout(url, options = {}, timeout = 30000) {
    ...
    try {
        ...
        throw new NetworkError(`网络连接失败: ${errMsg}`, e);
    }
}
...
const API = {
    ...
    getModVersions: (projectId, source = 'modrinth', loader = '', gameVersion = '') => {
        ...
        return fetchWithTimeout(`${getApiBase()}/api/mods/versions?${query}`, {}, 60000).then(async res => {
    ...
    // === MciLm-link 内网映射联机 ===
    mcilmStatus: () => apiGet('/api/mcilm/status'),
    mcilmHost: (gamePort, opts = {}) => apiPost('/api/mcilm/host', { gamePort, ...opts }),
    mcilmGuest: (shareCode, opts = {}) => apiPost('/api/mcilm/guest', { shareCode, ...opts }),
    mcilmStop: () => apiPost('/api/mcilm/stop', {}),
    mcilmInstalled: () => apiGet('/api/mcilm/installed'),
    mcilmInstall: () => apiPost('/api/mcilm/install', {}),
    mcilmUninstall: () => apiPost('/api/mcilm/uninstall', {}),
    mcilmVersion: () => apiGet('/api/mcilm/version'),
    mcilmRuntimeStatus: () => apiGet('/api/mcilm/runtime-status'),
    mcilmSwitchRelay: () => apiPost('/api/mcilm/switch-relay', {}),
    mcilmRuntimeVersion: () => apiGet('/api/mcilm/runtime-version'),
    mcilmLog: () => apiGet('/api/mcilm/log'),
    mcilmProbe: () => apiPost('/api/mcilm/probe', {}),
